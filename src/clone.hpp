...
public:
    ...
    */
     ...
     */
     void grow_with_dsb(const Cell_ptr ncell, const Model& model, int Nend, int mean_local_frag, int track_all = 0, int verbose = 0, int restart = 1, double tend = DBL_MAX){
         ...
         while(this->curr_cells.size() < Nend) {
             ...
             if(rb < rbrate){
                 ...
                 // this->n_cycle++;
                 rcell->do_cell_cycle(dcell1, dcell2, n_telo_fusion, n_complex_path, n_path_break, mean_local_frag, verbose);
